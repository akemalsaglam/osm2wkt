...
public class CustomOsm2Wkt {
    ...
    private boolean readOsm(String filename) {
        ...
        try {
            ...
            System.out.println("reading all landmarks.");
            NodeList landmarkList = doc.getElementsByTagName(XML_TAG_NODE);
            Stream<Node> landmarkNodeStream = IntStream.range(0, landmarkList.getLength()).mapToObj(landmarkList::item);

            ForkJoinPool customThreadPool = new ForkJoinPool(1);
            customThreadPool.submit(() -> landmarkNodeStream.parallel().forEach(landmark -> {
                Node markNode = landmark;
            ...
            // read in all streets
            System.out.println("reading all ways.");
            ...
            for (int s = 0; s < wayList.getLength(); s++) {
                ...
                for (int t = 0; t < ndList.getLength(); t++) {
                    ...
                    if (!ndNode.getNodeName().equalsIgnoreCase(XML_TAG_ND)) continue;
            ...
            wayListNodeStream.parallel().forEach(way -> {
                ...
                if (wayNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (idAttr == null) {
                        ...
                        for (int t = 0; t < ndList.getLength(); t++) {
                            Node ndNode = ndList.item(t);
                            if (ndNode != null && ndNode.getNodeType() == Node.ELEMENT_NODE && ndNode.getNodeName().equalsIgnoreCase(XML_TAG_ND)) {
    ...
    private boolean fixCompleteness() {
        ...
        List<Long> notExitStreets = new ArrayList<>();
       ...
       /* for (Map.Entry<Long, Vector<Long>> entry : streets.entrySet()){

        }*/

        //if(notExitStreets.size()>0) return false;
    ...
    private Landmark checkCrossing(Landmark a1, Landmark a2, Landmark b1, Landmark b2) {
        ...
        x = CustomOsm2Wkt.round(x, CustomOsm2Wkt.precisonFloating);
        //System.out.println("x = " + x);

        double y = (aA * bC - bA * aC) / det;
        y = CustomOsm2Wkt.round(y, CustomOsm2Wkt.precisonFloating);
    ...
    private double plainDistance(double x1, double y1, double x2, double y2) {
        ...
        distance = CustomOsm2Wkt.round(distance, CustomOsm2Wkt.precisonFloating);
        return distance;
    }
    ...
    private boolean simplifyGraph(boolean repair) {
        ...
        /*List<Long> removeLandmarks = new ArrayList<>();
        List<Long> removeStreets = new ArrayList<>();

        ForkJoinPool customThreadPool = new ForkJoinPool(10);
        try {
            customThreadPool.submit(() -> {
                verticesRemove.parallelStream().forEach(vertice -> {
                boolean removedL;
                do {
                    // remove all landmark
                    removedL = false;
                    for (Long lid : landmarks.keySet()) {
                        if (((long) lid) == ((long) vertice)) {
                            //landmarks.remove(lid);
                            removeLandmarks.add(lid);
                            System.out.println("landmark " + lid + " is removing.");
                            removedL = true;
                            break;
                        }
                    }
            ...
            if (customThreadPool != null) {
                customThreadPool.shutdown(); //always remember to shutdown the pool
            }
        }

        landmarks.keySet().removeAll(removeLandmarks);
        streets.keySet().removeAll(removeStreets);*/
    ...
    private boolean writeWkt(String wktfile, boolean append) {
        ...
        try {
            ...
            String path = util.FileReader.class.getClassLoader().getResource("taxidata").getPath();
            File wkt = new File(path + "/map.osm.wkt");
            if (!append) {
    ...
    public static void main(String[] args) {
        ...
        CustomOsm2Wkt obj = new CustomOsm2Wkt();
